...
/**
 * <p><code>NavigableImagePanel</code> is a lightweight container displaying an image that can be zoomed in and out and
 ...
 * </pre>
 *
 * <p>When an image is set, it is initially painted centered in the component, at the largest possible size, fully
 * visible, with its aspect ratio is preserved. This is defined as 100% of the image size and its corresponding zoom
 * level is 1.0.</p>
 *
 * <h3>Zooming</h3>
 *
 * <p>Zooming can be controlled interactively, using either the mouse scroll wheel (default) or the mouse two buttons,
 * or programmatically, allowing the programmer to implement other custom zooming methods. If the mouse does not have a
 * scroll wheel, set the zooming device to mouse buttons:</p>
 *
 * <pre>
   panel.setZoomDevice(ZoomDevice.MOUSE_BUTTON);
 * </pre>
 *
 * <p>The left mouse button works as a toggle switch between zooming in and zooming out modes, and the right button
 * zooms an image by one increment (default is 20%). You can change the zoom increment value by:</p>
 *
 * <pre>
   panel.setZoomIncrement(newZoomIncrement);
 * </pre>
 *
 * <p>If you intend to provide programmatic zoom control, set the zoom device to none to disable both the mouse wheel
 * and buttons for zooming purposes:</p>
 *
 * <pre>
   panel.setZoomDevice(ZoomDevice.NONE);
 * </pre>
 *
 * <p>and use <code>setZoom()</code> to change the zoom level.</p>
 *
 * <p>Zooming is always around the point the mouse pointer is currently at, so that this point (called a zooming center)
 ...
 * when <code>setZoom()</code> is called:</p>
 *
 * <pre>
   panel.setZoom(newZoomLevel, newZoomingCenter);
 * </pre>
 *
 * <p>or assumed to be the point of an image which is the closest to the center of the panel, if no zooming center is
 * specified:</p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>There are no lower or upper zoom level limits.</p>
 *
 * <h3>Navigation</h3>
 *
 * <p><code>NavigableImagePanel</code> does not use scroll bars for navigation, but relies on a navigation image located
 ...
 *
 * <p>In order to adjust the position of an image in the panel, it can be dragged with the mouse, using the left button.
 * </p>
 *
 * <p>For programmatic image navigation, disable the navigation image:</p>
 *
 * <pre>
   panel.setNavigationImageEnabled(false)
 * </pre>
 *
 * <p>and use <code>getImageOrigin()</code> and <code>setImageOrigin()</code> to move the image around the panel.</p>
 *
 * <h3>Rendering</h3>
 *
 * <p><code>NavigableImagePanel</code> uses the Nearest Neighbor interpolation for image rendering (default in Java).
 ...
 * the value of <code>HIGH_QUALITY_RENDERING_SCALE_THRESHOLD</code>.</p>
 *
...
public class NavigableImagePanel extends JPanel {
    ...
    /**
     * <p>Creates a new navigable image panel with no default image and the mouse scroll wheel as the zooming
     * device.</p>
     */
...
//        }
    ...
    public static class ZoomDevice {
        ...
        /**
         * <p>Identifies that the panel does not implement zooming, but the component using the panel does (programmatic
         * zooming method).</p>
         */
